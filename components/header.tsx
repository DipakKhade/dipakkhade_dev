...
const Header: FC<HeaderProps> = () => {
  ...
  return (
    <header className="z-[777] relative">
      <motion.div
      ...
      <nav className="flex fixed top-[0.15rem] left-1/2 h-12 -translate-x-1/2 py-2">
          ...
          <ul className="flex w-[22rem] flex-wrap items-center justify-center gap-y-1 text-[0.9rem] font-medium flex-[1/4] sm:p-0">
            {links.map((link) => (
              ...
              >
                <Link
                  className={clsx({
                    "flex w-full items-center justify-center px-3 py-1 hover:text-gray-950 transition dark:hover:text-gray-300":
                      true,
        ...
        <div>
          <motion.ul
            className="sm:flex hidden space-x-5 items-center md:mt-0 lg:mb-6 text-sm font-medium text-gray-500  sm:mb-0"
            initial={{ y: -100, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
          >
